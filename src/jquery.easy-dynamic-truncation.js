...
;(function( $, window, document, UNDEFINED ){
    ...
    var FALSE = false,
        ...
        $truncate = $( [
            toDataAttribute( MAX_ITEMS ),
            toDataAttribute( CHAR_LIMIT )
        ].join(',') ),
    ...
    {
        return DATA_ATTR + key + CLOSE_SQUARE;
    }

    function findChildren($el, exclude)
    {
        return ( exclude != UNDEFINED ? $el.children( NOT + exclude + CLOSE_PAREN ) : $el.children() )
    ...
    $truncate.each(function(){
        ...
        // should user be able to re-collapse?
            collapsible = $this.is( toDataAttribute( PREFIX + '-collapsible' ) ),
        // should we display "more"?
            no_more = $this.is( toDataAttribute( PREFIX + '-no-more' ) ),
        // character limit?
            char_limit = FALSE,
            char_total = 0,
            $string = $([]),
        // temp child (if we need it)
            temp_child = false,
        // does more exist already?
            embed = $this.is( toDataAttribute( PREFIX + '-embed' ) ),
        // does more exist already?
            $more_link = $this.siblings( MORE_SELECTOR ),
            has_link = ( $more_link.length > 0 ),
            $link,
        // find the kids
            $children = findChildren( $this, exclude );


        // Character limit
        if ( $this.is( toDataAttribute( CHAR_LIMIT ) ) )
        {
            ...
            {
                temp_child = true;
                $this.contents().wrap( '<b class="temp"/>' );
                ...
                if ( ( char_total + length ) > char_limit )
                {
                    ...
                    $.each( words, function( i, word ){
                        ...
                        if ( char_total + word_length > char_limit )
                        {
                            ...
                            search = '';
                            if ( words[i-1] ) {
                                search = words[i-1] + SPACE;
                            }
                            if ( words[i-2] ) {
                                search = words[i-2] + SPACE + search;
                            }
            ...
            if ( temp_child )
            {
                $this.find('.temp').contents().unwrap();
                $children = findChildren( $this, exclude );
            }
